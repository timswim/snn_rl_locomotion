"""
Script to train hybrid SNN actor + ANN critic RL agent with torch.
...
parser = argparse.ArgumentParser(description="Train a hybrid SNN actor + ANN critic RL agent with torch.")
...
from models.ANN_SNN_PPO import (
    ActorCritic,
    compute_gae,
    detach_state,
    initial_zero_hidden,
...
def train_one_run(
    ...
    current_actor_state = initial_zero_hidden(model, num_envs, num_inputs, device)
    ...
    try:
        while step_idx < hyperparams.max_steps:
            ...
            for _ in range(hyperparams.num_steps):
                ...
                dist, value, current_actor_state, _ = model(state, current_actor_state)
                ...
                # Среда перезапускает эпизод: обнуляем скрытое состояние SNN-актера для этих env,
            ...
            _, next_value, _, _ = model(next_val_state, current_actor_state)
...
def main():
    ...
    try:
        run_name = args_cli.run_name or ("train_ann_snn_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
