"""
Optuna hyperparameter search for train_ann_snn.py - one trial per process.
...
Metric is passed via file (OPTUNA_METRIC_FILE). Run from the scripts/ directory:
    ...
    cd scripts && python optuna_tune_ann_snn.py --task=Isaac-Velocity-Flat-Unitree-A1-v0 --n_trials=20
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Optuna tuning for train_ann_snn.py (one trial per subprocess)."
    )
    ...
    p.add_argument("--num_envs", type=int, default=None, help="Number of envs (default from train_ann_snn).")
...
def run_trial(
    ...
    """Run train_ann_snn.py in a subprocess; stdout/stderr go to terminal. Metric is read from file."""
    argv = [
        python_exe,
        os.path.join(script_dir, "train_ann_snn.py"),
        "--task", task,
        "--seed", str(seed),
        "--run_name", run_name,
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                "[optuna_tune_ann_snn] Trial subprocess exited with code %d, returning bad metric %.2f"
                % (result.returncode, bad_metric),
                file=sys.stderr,
            )
        else:
            print(
                "[optuna_tune_ann_snn] OPTUNA_METRIC_FILE missing or empty, returning bad metric %.2f"
                % bad_metric,
                file=sys.stderr,
            )
        return bad_metric
